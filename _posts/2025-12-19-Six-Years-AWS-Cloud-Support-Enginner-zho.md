...
2018 年大三時，我經由系上一位學長的介紹，有機會到一家名為 Larvata 的軟體公司擔任 Python 開發實習生。有趣的是，我的導師同時也是 DevOps 團隊的 team lead。在實習期間，我看到了他們如何進行自動化、監控與容器相關的工作，也觀察到 Kubernetes 是如何被導入實際環境中。我的導師也很樂於分享他的工作流程，包括在 command line 與 Vim 上的操作。當時我心裡只覺得：「這真的太酷了。」
...
我從 EC2-Linux 開始，逐步深入理解 Linux，以及底層硬體與虛擬化的運作方式。我遇到非常棒的導師與資深同事，在每一場知識分享中都學到了很多。
...
我記得有一次假期期間，一個緊急的網路案例被升級，但當天沒有其他資深網路工程師值班，我必須站出來處理。即使在我處理這個案例的階段並沒能直接找出根本原因，我仍完成了初步排查並成功升級至內部團隊，直到其他資深的技術支援工程師接手深入調查。
...
排查一個案例，就像醫師看診一樣。我們需要先確認客戶主觀描述的狀況如痛點、影響、時間、資源等（Subjective），再蒐集客觀資料，例如數據、log、截圖（Objective）。
...
我曾多次看到優秀的同事直接引用 RFC 規格或開源程式碼向客戶解釋問題。也有不少情況，是同事透過完整的實驗步驟與結果，回饋給內部團隊，協助修正文件中的錯誤。
...
當我離開 AWS 時，我道別的人包含：同儕、資深與 Principal 技術支援工程師、團隊內外的主管、架構師、TPM、TAM，以及來自 AMER / EMEA / APJC 的同事。能夠認識並與這些人共事、向他們學習，我感到非常幸運。
...
Amazon 非常重視寫作。內部溝通不使用 PowerPoint，這在一開始讓我感到非常驚訝，但後來我發現這確實是正確的方式。無論是一頁或六頁文件，寫作能幫助釐清思路。當我想找某個提案時，會直覺地去找對應的 initiative 或 PRFAQ。在會議中，大家安靜地花 15 分鐘閱讀文件，然後提出意見。

以之前我嘗試提出如何改善工程師工作環境的方案時，也有寫一個一頁文件 (one-page)，在會議中主管以及團隊成員便會一起坐下來閱讀我寫的文件，大家會在文件上寫下評論和注解：這句話是什麼意思？數據怎麼收集的？爲什麼可以支撐你的論點？有更好檢視預期結果的方式嗎？等等。我便會再回頭檢視和修改這個文件。

這是一個非常有價值的過程。我能從中學到文件缺少了什麼、該如何改進，以及如何讓提案更完整。

## 結語

感謝你看完我這段 AWS 的旅程。離開 AWS 的決定並不容易，心情也非常複雜。有時在每天處理案例的過程中，我覺得自己學到了很多；但有時又覺得現階段對於職涯成長需要不同的刺激。

我很感謝在 AWS 中的成長和學習，但同時也意識到需要不同的環境，來繼續探索與累積經驗。我期待人生的下一個章節，也期待能繼續和大家一起學習。
